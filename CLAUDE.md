...
Six modules are public surface: `Xcribe`, `Xcribe.Document`, `Xcribe.Information`, `Xcribe.Formatter`, `Xcribe.Web.Plug`, `Mix.Tasks.Xcribe.Doc`. Also public by contract: the config keys documented in `Xcribe`'s `@moduledoc`, the `document/1,2` entries exported through `.formatter.exs` `locals_without_parens`, and the shape of the generated `.apib`/JSON output. **Everything else is `@moduledoc false` and free to change.** Full rule: `.claude/rules/public-api.md`.
...
- `public-api.md` — the six public modules, `@moduledoc false` as the default, `@doc false`, what counts as a breaking change.
